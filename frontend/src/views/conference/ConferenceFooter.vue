<template>
  <div id="footer">
    ...
    <!-- 마이크 음소거 -->
    <button class="btn_center_1" type="button" @click="$emit('audioOnOff'), audioIcon()">
    ...
    <!-- 내 얼굴 영상 공유 중지 -->
    <button class="btn_center_2" type="button" @click="$emit('videoOnOff'), videoIcon()">
    ...
    <!-- 컴퓨터 화면 공유 -->
    <button class="btn_center_3" type="button" @click="$emit('shareScreen')">
    ...
    <!-- 드로잉 버튼 -->
    <button class="btn_center_4" type="button" @click="$emit('shareDrawing'), drawIcon()">
  ...
  <div id="sideBar">
    <button type="button" @click="$emit('userListOnOff'), valueOnOff(usersValue)">
    ...
    <button type="button" @click="$emit('chattingOnOff'), valueOnOff(chatValue)">
    ...
    <!-- 나가기 -->
    <button  type="button" class="leave-meeting" @click="$emit('sessionExit')"><span class="material-symbols-outlined">logout</span></button>
  </div>
  </div>
</template>
...
button.btn_center_1{
  ...
  box-shadow: 0 4px 16px rgba(231,19,217,0.3);
}

button.btn_center_2{
  color: rgba(158,83,228,0.7);
  background-color: white;
  box-shadow: 0 4px 16px rgba(158,83,228,0.3);
}

button.btn_center_3{
  color: rgba(72,159,242,0.7);
  background-color: white;
  box-shadow: 0 4px 16px rgba(72,159,242,0.3);
}

button.btn_center_4{
  color: rgba(0,234,254,0.7);
  background-color: white;
  box-shadow: 0 4px 16px rgba(0,234,254,0.3);
}
